...
#endif
#define JOYSTICK_MIN_THREHOLD 1200
#define JOYSTICK_MAX_THREHOLD 3200
static volatile uint8_t keypad_value;
static volatile uint8_t keypad_released;
static volatile uint8_t key_laserfire;
static volatile uint8_t sendcommand_request;
static volatile int16_t laserpower_range;
// static volatile uint8_t key_laser_fire_released;
static uint16_t joyStick_X_valueBuffer[12];
static uint16_t joyStick_Y_valueBuffer[12];
// static bool f;
...
#define keypad_print_int(int) print_int(keypad_cmd_putc, int)

bool keypad_process(void *args)
{
	if (io_get_input(JOYSTICK_EN))
	{
	...
	if (cnt < 12)
	{
		...
		// if (cnc_get_exec_state(EXEC_IDLE))
		// {
		joyStick_X_valueBuffer[cnt] = io_get_analog(JOYSTICK_X);
		joyStick_Y_valueBuffer[cnt] = io_get_analog(JOYSTICK_Y);
		cnt++;
		// }
		// }
	...
	// else if (cnt < 24)
	// {
	// 	joyStick_Y_valueBuffer[cnt - 12] = io_get_pinvalue(JOYSTICK_Y);
	// 	if (cnt == 12)
	// 	{
	// 		valuex = mdAVFilter(joyStick_X_valueBuffer);
	// 	}
	// 	cnt++;
	// 	return EVENT_CONTINUE;
	// }
	else
	{
		cnt = 0;
		valuex = mdAVFilter(joyStick_X_valueBuffer);
		valuey = mdAVFilter(joyStick_Y_valueBuffer);
		// valuey = 450;
		if (valuex < JOYSTICK_MIN_THREHOLD && valuey < JOYSTICK_MIN_THREHOLD)
		{
			keypad_value = 't';
			keypad_released = 0;
		} // X-Y-
		else if (valuex < JOYSTICK_MIN_THREHOLD && valuey > JOYSTICK_MAX_THREHOLD)
		{
			keypad_value = 's';
			keypad_released = 0;
		} // X-Y+
		else if (valuex > JOYSTICK_MAX_THREHOLD && valuey < JOYSTICK_MIN_THREHOLD)
		{
			keypad_value = 'q';
			keypad_released = 0;
		} // X+Y-
		else if (valuex > JOYSTICK_MAX_THREHOLD && valuey > JOYSTICK_MAX_THREHOLD)
		{
			keypad_value = 'r';
			keypad_released = 0;
		} // X+Y+
		else if (valuex < JOYSTICK_MIN_THREHOLD)
		{
			keypad_value = 'L';
			keypad_released = 0;
		} // X-
		else if (valuex > JOYSTICK_MAX_THREHOLD)
		{
			keypad_value = 'R';
			keypad_released = 0;
		} // X+
		else if (valuey < JOYSTICK_MIN_THREHOLD)
		{
			keypad_value = 'F';
			keypad_released = 0;
		} // Y-
		else if (valuey > JOYSTICK_MAX_THREHOLD)
		{
			keypad_value = 'B';
			keypad_released = 0;
		} // Y+
		else if (!io_get_input(JOYSTICK_Z_DOWN))
		{
			keypad_value = 'D';
			keypad_released = 0;
		}
		else if (!io_get_input(JOYSTICK_Z_UP))
		{
			keypad_value = 'U';
		}
		else
		...
		if (!io_get_input(LASER_FIREONCE))
		{
			if (!key_laserfire)
			{
				if (mcu_millis() - debounce > 10)
				{
					...
					laserpower_range = tool_get_setpoint();
					if (laserpower_range < 10)
						laserpower_range = 10;
					tool_set_speed(1000);
					// sendcommand_request = 1;
				}
				else
					key_laserfire = 0;
			}
	...
	else if (feed != 0)
	{
		...
		// if (key_laserfire && sendcommand_request)
		// {
		...
		// 	keypad_cmd_putc('\n');
		// 	keypad_cmd_ptr = &cmd[36];
		// 	keypad_print_str("M3S");
		// 	keypad_print_int(laserpower_range);
		// 	keypad_cmd_putc('\n');
		// 	keypad_stream_try_enqueue_cmd(cmd);
		// 	DEBUG_STR(cmd);
		// 	sendcommand_request = 0;
		// }
		// else
		// {
		memset(cmd, 0, RX_BUFFER_SIZE);
		...
		keypad_stream_try_enqueue_cmd(cmd);
		// DEBUG_STR(cmd);
		// serial_print_str(cmd);
		// }
	}
	// else if (key_laserfire && sendcommand_request)
	// {
	// 	keypad_value = 0;
	// 	memset(cmd, 0, RX_BUFFER_SIZE);

	// 	rom_strcpy(cmd, __romstr__("M3S"));
	// 	keypad_cmd_ptr = &cmd[3];
	// 	keypad_print_int(laserpower_range);
	// 	keypad_cmd_putc('\n');
	// 	keypad_stream_try_enqueue_cmd(cmd);
	// 	sendcommand_request = 0;
	...
	// 	rom_strcpy(cmd, __romstr__("M5S0\n"));
	// 	keypad_stream_try_enqueue_cmd(cmd);
	// 	sendcommand_request = 0;
	// }
	else
...
#endif
// bool keypad_laserfire(void *args)
// {
// 	static uint32_t debounce = 0;
...
// 		keypad_stream_try_enqueue_cmd(cmd);
// 		sendcommand_request = 0;
// 		serial_stream_readonly(keypad_stream_getc, keypad_stream_available, keypad_stream_clear);
...
// 	}
// }
// CREATE_EVENT_LISTENER(cnc_dotasks, keypad_laserfire);
...
//     protocol_send_string("keypad_v:");
// 	serial_print_int(keypad_value);
// 	protocol_send_string("keypad_r:");
// 	serial_print_int(keypad_released);
...
DECL_MODULE(grblhal_keypad)
{
	...
	// ADD_EVENT_LISTENER(cnc_dotasks, keypad_laserfire);
